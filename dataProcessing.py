...
repStations = ['Choa Chu Kang', 'Jurong East', 'Buona Vista',
               'Harbour Front', 'Outram Park', 'Dhoby Ghaut',
               'Marina Bay', 'Raffles Place', 'Bishan',
               'Serangoon', 'Paya Lebar', 'Punggol',
               'Pasir Ris', 'Changi Airport', 'Joo Koon']  # 16

repStations = ['Buona Vista',
               'Harbour Front', 'Outram Park', 'Dhoby Ghaut',
               'Marina Bay', 'Raffles Place', 'Bishan',
               'Serangoon', 'Paya Lebar']  # 9
...
def gen_flowWrs(openBrowser=True):
    ...
    with open(csv_fpath, 'wt') as w_csvfile:
        writer = csv.writer(w_csvfile, lineterminator='\n')
        header = ['fSTN', 'tSTN', 'count', 'path']
        writer.writerow(header)
    #
    df = pd.read_csv(opath.join(dpath['flow'], 'flow-M%d%02d.csv' % (year, month)))
    for fSTN, tSTN, count in df[['fSTN', 'tSTN', 'count']].values:
        if fSTN not in repStations or tSTN not in repStations:
            continue
        if fSTN == tSTN:
            continue
        coords, path = [], []
