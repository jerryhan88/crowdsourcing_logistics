...
def subtourelim(model, where):
    if where == GRB.callback.MIPSOL:
        selected = []
        for i in range(n):
            sol = model.cbGetSolution([model._x_ij[i, j] for j in range(n)])
        ...
        if len(min_subtour) < n:
            expr = 0
            for i in range(len(min_subtour)):
                for j in range(i + 1, len(min_subtour)):
...
for i in range(n):
    m.addConstr(quicksum(x_ij[i, j] for j in range(n)) == 2)
    x_ij[i, i].ub = 0
